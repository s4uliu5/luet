...
curl -L https://github.com/${GITHUB_USER}/luet/releases/download/${LUET_VERSION}/luet-${LUET_VERSION}-Linux-${LUET_ARCH} --output /usr/bin/anise
...
if [ "${LUET_ARCH}" = "x86_64" ] ; then
  ...
  anise install --sync-repos -y system/luet-${GITHUB_USER} --force
else
